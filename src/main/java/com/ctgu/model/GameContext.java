...
public class GameContext
{
  ...
  private String transChessName(Position position, Move move)
  {
    ...
    case 2:
      ...
      for(index = 0; index < leftColSize; index++)
      {
        ...
        {
          sb.append(ChessConstant.DIGITS[2 - side].charAt((side == 0 ? index : (leftColSize - index) - 1) + rightColSize))
              .append(chessName);
          return sb.toString();
        }
      }
  ...
  public boolean saveManual(String filePath)
  {
    synchronized(this)
    {
      ...
      try
      {
        ...
        {
          logger.log(Level.WARNING, "Failed while writing manual", e);
          return false;
        }
      }
      catch(Exception e)
      {
        logger.log(Level.WARNING, "Failed to create manual file stream", e);
        return false;
      }
    }
  }
  ...
  private void writeManual(PrintWriter writer)
  {
    ...
    for(int i = 0; i < moveCount; i++)
    {
      if(i % 2 == 0)
      {
        writer.format(" %d. %s ", Integer.valueOf((i / 2) + 1), moveStringList.get(i));
      }
      else
      {
        writer.format(" %s \n", moveStringList.get(i));
      }
    }
    writer.println(" *");
  }
  ...
  {
    return "GameContext [initFen=" + initFen + ", fenList=" + fenList + ", positionList=" + positionList + ", moveList=" + moveList
        + ", moveStringList=" + moveStringList + "]";
  }
}
